...
We use `faster-whisper` (CTranslate2-based) instead of the reference Whisper
implementation: 3–4x faster at the same quality. Defaults to the `medium`
multilingual model, which covers Italian and English well enough for
hackathon-quality lecture audio.

Returns a list of `Segment` dicts with `t_start`, `t_end`, and `text`. These
become `Block` instances with `TimeLocator` in `agent.parsers.audio_parser`.
"""
